...
class rebase_meta(type):
    ...
    def __init__(cls, cls_name, cls_bases, cls_dct):
        ...
        bases = list(rebase_meta.base_classes_subs.items())
        ...
        for cls_base in cls_bases:
            if(cls_base.__name__ not in rebase_meta.base_classes_subs):
        ...
        if(cls_name in rebase_meta.base_classes): # Is class a base class
            ...
            for (sub_cls_name, sub_cls) in list(subs.items()):
                ...
                try:
                    ...
                    if(rebase_meta.mod_loaded):
                        new_sub_cls = rebase_meta(sub_cls_name, sub_bases, dict(list(sub_cls.__dict__.items()) + [("__module__", rebase_meta.mod_name)]))
                        rebase_meta.mod_name_space[sub_cls_name] = new_sub_cls
                    else:
                        new_sub_cls = rebase_meta(sub_cls_name, sub_bases, dict(list(sub_cls.__dict__.items())))
...
org_class_types = [] #[x for x in list(types.__builtins__.values()) if isinstance(x, type) and (not x.__name__ in forbidden_subs) and x.__module__ == "__builtin__"]
...
class mr_agreeable(mr_quiet):
    ...
    def prognogsis(self):
        self.sin_counter += 1
        self.nutty_factor = max([x for x in list(self.rdict.keys()) if x < self.sin_counter])
    def insanity(self):
        insane_strs = \
        [
...
def transform_base_to_sub(idict):
    ...
    '''
    sub_dict = {}
    classes = list(idict.keys())
    for cur_class in idict:
        sub_dict[cur_class] = [cls for cls in classes if cur_class in idict[cls]]
    return sub_dict

recur_class_helper = lambda idict, ilist = []: [[key, recur_class_helper(idict, idict[key])] for key in ilist]
recur_class = lambda idict: recur_class_helper(idict, list(idict.keys()))
...
class proc_func(list):
    ...
    def get_args(self, *args):
        self.extend([x for x in args if x])
...
class proc_inputs(proc_func):
    def get_args(self, *args):
        self.extend(list(filter(os.path.isfile, args)))
...
if __name__ == "__main__":
    ...
    if(len(proc_input) != len(proc_output)):
        ...
        in_out_pairs = list(zip(proc_input, proc_output))
        for (in_file, out_file) in in_out_pairs:
            ...
            exclusions = list(name_space.keys())

            # Associate the fake name space to the rebasing metaclass
            rebase_meta.register_mod(dodgy_module_name, name_space)

            # Run dodgy code
#            exec(compile(open(in_file).read(), in_file, 'exec'), name_space)

            # Bring back dodgy classes
            import_classes = [cls if(isinstance(cls, type) and not cls_name in exclusions) else None for (cls_name, cls) in list(name_space.items())]
            dodgy_import_classes = [x for x in import_classes if x]
            ...
            base_set = reduce(lambda x, y: x | y, list(map(set, list(base_dict.values()))), set([]))
            sub_set = reduce(lambda x, y: x | y, list(map(set, list(sub_dict.values()))), set([]))
            ...
            inbred_lines = [[x for x in lineage if x.__module__ == dodgy_module_name] for lineage in lineages]
